...
函数说明：
    ...
    4）创建每一层的权重weights
      ...
      注意：出于实际工作需要，另外追加了xavier初始化方式，如果需要其它初始化方式请自行添加
...
def conv(x, filter_height, filter_width, num_filters, stride_y, stride_x, name,
  ...
  with tf.variable_scope(name) as scope:
    ...
    return relu
### Helpers
def _weights(name, shape, initializer='xavier',mean=0.0, stddev=0.02):
  ...
  #contrib源码显示，xavier的底层也是调用的variance_scaling_initializer
  elif initiallizer =='xavier':
  	var = tf.get_variable(name, shape,initializer=tf.contrib.layers.xavier_initializer())
  elif initiallizer =='scaling':
    var = tf.get_variable(name, shape,initializer=tf.contrib.layers.variance_scaling_initializer())
  return var
